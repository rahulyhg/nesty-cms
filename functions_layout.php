...
function login($disclaimer=null) {
	...
	if (!(empty($recaptcha_site)) && !(empty($recaptcha_private)) && ($recaptcha_override !== "yes")):
		...
		echo "<button id='submit_button' type='submit' name='login' value='login' class='gray_background' disabled>login</button>";
	else:
		echo "<button id='submit_button' type='submit' name='login' value='login'>login</button>";
		endif;
