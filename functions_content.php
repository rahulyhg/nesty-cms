...
$sql_temp = "SELECT * FROM $database.entries WHERE entry_id=:entry_id ORDER BY name ASC, year ASC, month ASC, day ASC";
...
//	preg_match_all("/(?<=\{\{\{)(.+)(?=\}\}\})/is", $body_incoming, $matches); // too greedy
	...
	foreach ($matches as $match_temp):
		...
		elseif ($filename_size == "large"):
			...
			$image_string .= "<figure><amp-img on='tap:lightbox".$media_id_temp."' src='".$media_info[$media_id_temp]['link']."large/' width='".$img_width_large."px' height='".$img_height_large."px' role='button' tabindex='1' sizes='(min-width: 1000px) 900px, (min-width: 500px) 90vw, 450px'></amp-img>";	
